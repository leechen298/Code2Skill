...
export const lookupTopicInputSchema = z.object({topic: z.string().min(1)}).strict();
export const lookupTopicOutputSchema = z.object({
  status: z.literal('success'),
  data: z.object({topic: z.string()})
});
...
server.registerTool("lookup_topic", {
  ...
  inputSchema: lookupTopicInputSchema.shape,
  outputSchema: lookupTopicOutputSchema.shape,
    ...
    for await (const line of lines) {
      ...
      if (message.method === 'initialize') {
        ...
        try {
          ...
          for (const [key, schema] of Object.entries(tool.config.inputSchema)) schema.parse(args[key]);
          send({jsonrpc: '2.0', id: message.id, result: await tool.callback(args, {})});
        } catch (error) {
...
def create_candidate(root: Path) -> Path:
    ...
    write(
        ...
        "  assert.deepEqual(await lookupTopic({topic: 'alpha'}), "
    ...
    write(
        ...
        "      env: {...process.env, CODE2SKILL_DRY_RUN: '1'},\n"
...
class CoreExportValidatorTest(unittest.TestCase):
    ...
    def test_accepts_small_runnable_delivery_without_audit_files(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            self.assertIn("1 tools", result.stdout)
    ...
    def test_tests_must_reference_every_function_and_tool(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            candidate = create_candidate(Path(directory))
            write(
                candidate / "tests" / "function.test.mjs",
                "// lookupTopic lookup_topic tools/list tools/call structuredContent isError\n"
                "import { test } from 'node:test';\ntest('placeholder', () => {});\n",
            )
            (candidate / "tests" / "mcp.test.mjs").unlink()
            result = run_validator(candidate)
            self.assertEqual(result.returncode, 1)
            self.assertIn("tests: MCP tests must reference Tool lookup_topic", result.stderr)
            self.assertIn("tests: Function tests must import lookupTopic", result.stderr)
    ...
    def test_rejects_schemas_that_are_declared_but_not_executed(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            candidate = create_candidate(Path(directory))
            ...
            result = run_validator(candidate)
            self.assertEqual(result.returncode, 1)
            self.assertIn("input and output schemas must execute at runtime", result.stderr)
    ...
    def test_rejects_error_result_marked_as_success(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            result = run_validator(candidate)
            self.assertEqual(result.returncode, 1)
            self.assertIn("toMcpResult(..., true)", result.stderr)
    ...
    def test_requires_reviewed_error_normalizer(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            candidate = create_candidate(Path(directory))
            ...
            result = run_validator(candidate)
            self.assertEqual(result.returncode, 1)
            self.assertIn("reviewed shared runtime", result.stderr)
