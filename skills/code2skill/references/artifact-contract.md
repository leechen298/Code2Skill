...
The bundle must agree deterministically with `canonical-contract.json`. Public inputs, requiredness, value domains, handoffs, failures, required outputs, side effects, operation policy, and hard prerequisites cannot diverge. Dynamic values scoped to identity, tenant, session, version, or freshness remain dynamic; one observed response must not become a fixed enum. Every structurally optional input (`required: false` with no `requiredWhen`) supplied by an observed upstream Tool distinguishes source-proven optionality from unproven target requiredness through the closed `targetRequiredness` decision, regardless of whether its information class is optional, derived, or dynamic. Matching handoffs and graph edges remain optional/non-hard. An unproven decision names the exact normal provider, needs request/call or behavior evidence from the feature boundary's primary source rather than supplementary backend or interface-contract evidence alone, remains optional in executable Schemas, appears in review items, and adds a controlled omission behavior check instead of becoming a hard Workflow.
...
Each Capability row also contains a deterministic `reviewItems` array. Its `issueRef` values point to exact `canonical-contract.json` `missingEvidence`, unproven input `targetRequiredness`, unresolved-conflict locations, or exact `host-compatibility-report.json` requirement restrictions. `approval-audit.json.capabilities[]` repeats only the row decision, exact `reasons`, and those `issueRefs`; it is an audit projection, never a manually maintained uncertainty contract. Use empty `reviewItems` and `issueRefs` arrays when no concrete question exists.
