...
客户端若将选中结果的多个字段或整行对象展开、合并进下游请求，把它建模为选中记录交接。下游只读少数字段时逐项公开；确实转发整行时使用业务命名的开放对象（如 `selectedOrder`）。Function 按源码顺序合并且保持显式覆盖关系。除非证据证明只传标识符等价，Skill 的标准示例必须传递该记录；省略能否接受仍交给后端判断。
...
1. Function 冒烟：按源码中明确可见的调用覆盖基本 method/URL/query/header/body 或 multipart 组装；至少用一个类型变体、`null` 或额外参数证明请求不会被业务 Schema 提前拦截，不扩展成业务规则组合矩阵。存在选中记录交接时，用两条不同的匿名记录切换调用，确认请求无前一条记录残值且覆盖顺序正确。
