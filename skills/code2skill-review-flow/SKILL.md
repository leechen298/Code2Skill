...
description: 从用户授权源码独立识别主要用户目标，轻量审核已有 core-export-v1 Code2Skill 包的代表性标准路径是否闭环；输出完整、基本可用、阻塞或未验证，不展开源码精确度审计。
---

# Code2Skill Main Flow Review

审核已有 `core-export-v1` 生成包能否帮助 Agent 沿代表性标准路径完成主要用户目标。它只回答“主流程是否基本可用”，不负责逐字段证明生成结果与源码完全一致。
...
只审核 `package.json` 声明为 `core-export-v1` 的包。遇到 strict、旧版或无法识别的包时，说明不适用，不要套用本流程。
...
2. Function/MCP 是否具备完成标准路径的必要能力；
