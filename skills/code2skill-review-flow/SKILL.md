...
2. Function/MCP 是否具备完成标准路径的必要能力，并清楚声明 Host 必须提供的业务服务基址（HTTP 场景下即 API 基址，RPC/Service/消息/任务场景下为对应客户端、Broker 连接或运行上下文）；
...
5. Agent 是否能构造终端业务调用（HTTP 场景下亦称终端请求），并根据实际响应继续沟通或停止。
